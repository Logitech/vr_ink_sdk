# Menu Interaction
![Example Buttons](../Images/Toolkit/MenuInteraction/ExampleButtonsPanel.png)

The Menu Interaction module provides a way to interact with 3D diegetic UI. Each UI element can be interacted through raycasting and controller button input or through physical interaction by pushing the UI element.
...
Each of the existing Toolkit UI elements is connected to a collider and Unity UI component.
<br>
![Example Toolkit UIInteractable](../Images/Toolkit/MenuInteraction/Inspector_ExampleUIInteractable.png)

### Animation
Animations are generally* handled through both the Unity UI component Transition and the `UIInteractable` 3D button and related fields. Each UI element has a animator attached which, for example, use the Unity Button Animation Transitions `Normal`, `Highlighted`, `Pressed` and `Disabled` to set the colour of the 3D Button. The `UIInteractable` fields can be used to handle how far the 3D Button raises when hovered along with the max distance it can be pushed, its click distance and the animation speeds.
...
If you need more granular input from the UI, some Unity Events are exposed in the inspector of each `UIInteractable`.
<br>
...
These methods are very similar to the public events and booleans mentioned earlier, but some provide extra information such as the the RaycastHit information if needed.
...
Tracked Device is the device used for input, and Stylus Button is the button used for input. By default, the Tracked Device is the primary device (the VR Ink if connected) and the Stylus Button is the primary button.
...
Block Raycast Triggers is a list of `PropertyStateTriggers`. In short, these can be assigned any component and will return the state of a chosen public boolean. If that state is true, it will prevent `RaycastClicker` from interacting with any UI elements. For example, we could prevent any unwanted button presses while drawing in the air.

## Button
A button is a `ButtonInteractable` which uses a Unity button component.
...
## Slider
A slider is a `SliderInteractable` which uses a Unity slider component.
...
## Toggle
A toggle, like a slider, is a `SliderInteractable` which uses a Unity slider component.
...
## Checkbox
A checkbox is a `ToggleInteractable` which uses a Unity toggle component.
...
## Radio Button
A Radio button, like a checkbox, is a `ToggleInteractable` which uses a Unity toggle component.
...
# UI Helpers
There are various scripts found under `Scripts/Components/UIHelpers` that can be used in conjunction with the Unity `OnClick()` and `OnValueChanged()` events. They are generally very small and prescribed, usually used just to toggle or set a value or ensure button animations work correctly.