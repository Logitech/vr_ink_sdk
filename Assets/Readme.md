...
<br>
[<img src="../Documentation/Images/LandingPage/Driver.png" width="290" alt="Demo Experience">](../Documentation/Driver)

## 2. Pairing VR Ink
1. First, turn on VR Ink by pressing the system button for about a second. The status LED should appear blue if it is not currently paired to a system.
2. In the SteamVR status window, right click on a controller icon and select 'Pair Controller'.
3. Press (and hold) both the Menu and Power buttons on VR Ink for a few seconds until the status LED starts blinking blue.
