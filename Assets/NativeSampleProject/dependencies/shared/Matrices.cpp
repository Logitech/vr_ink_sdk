...
Vector4 Matrix4::getQuaternion()
{
    Vector4 q;
    q.w = (float)(sqrt(fmax(0, 1 + m[0] + m[5] + m[10])) / 2);
    q.x = (float)(sqrt(fmax(0, 1 + m[0] - m[5] - m[10])) / 2);
    q.y = (float)(sqrt(fmax(0, 1 - m[0] + m[5] - m[10])) / 2);
    q.z = (float)(sqrt(fmax(0, 1 - m[0] - m[5] + m[10])) / 2);
    q.x = copysign(q.x, m[6] - m[9]);
    q.y = copysign(q.y, m[8] - m[2]);
    q.z = copysign(q.z, m[1] - m[4]);
    return q;
}
